...
# No SQL Similarity Database
try:
    SIM_DB = MongoClient(MONGO_CONNECT)[MONGO_DB]
except Exception as e:
    raise ConnectionError("Database connection error: ", e)


def load_mols_db(smiles_pickle, db_coll=SIM_DB["molecules"], fp_dict=FP_GENS, replace=False):
    # Get Dataset
...
def get_incomplete_ids(ref_df, db_coll=SIM_DB["mol_pairs"], expected_num=26583):
...
def add_db_idx(db_conn=SIM_DB, skip_existing=False, id_list=None):
...
def create_all_idx(db_coll=SIM_DB["mol_pairs"]):
...
def insert_db_data(_id, db_conn=SIM_DB, elec_props=ELEC_PROPS, sim_metrics=SIM_METRICS, fp_gens=FP_GENS, verbose=1, insert=True):
    # Get reference data
...
def batch_insert_db_data(ids, batch_size=100, db_coll=SIM_DB["mol_pairs"], **kwargs):
...
def create_db_compare_df_parallel(limit=1000, db_coll=SIM_DB["mol_pairs"], **kwargs):
    test_prop = "diff_homo"
